...
conda env create -f ~/06-Integration_Project-main/2-Conda_envs/trimmo.yml
conda activate trimmo
echo "Starting trimmomatic at: $(date)"
python3 ~/06-Integration_Project-main/3-TRIM/MASTERPROCESS_trimmomatic.py ${USER_ID}
echo "Finished trimmomatic at: $(date)"
conda deactivate

# Create reference for rsem/star

conda env create -f ~/06-Integration_Project-main/2-Conda_envs/rsem_env.yml
